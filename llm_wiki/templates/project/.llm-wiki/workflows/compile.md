...
3. 문서 생성/갱신을 결정한다:
   ...
   - **기존 문서가 `reviewed`/`approved`** → 본문을 건드리지 않고 `30_Wiki/_Proposals/<문서명>-<실행ID>-<원자료명>.md`에 제안 작성 (현재 내용 / 제안 내용 / 근거). 같은 이름의 파일이 이미 있으면 덮어쓰지 말고 `-2`, `-3`을 붙인다 — 한 실행에서 여러 자료가 같은 문서에 제안할 수 있다
...
6. **회의록인 경우**: 논의 내용은 위 절차대로 처리한다. **확정된 결정사항**은 결정 1건마다 `.llm-wiki/templates/decision.md` 형식의 완성본으로 `30_Wiki/_Proposals/decisions-<결정일> <결정 제목>.md`에 쓴다 (`40_Decisions`에 직접 쓰지 않는다). frontmatter `date`는 회의 날짜, `context`는 회의 이름이고 `recorded`·`recorded_by`는 비워 둔다 — 사람이 `llm-wiki review apply decisions-…`로 `40_Decisions`에 저장할 때 CLI가 채운다. 논의만 되고 확정되지 않은 것은 결정이 아니다.
...
8. **참고자료(`reference`)인 경우**: 외부 주체(기업·기관)가 만든 자료다. 여기서 나온 주장은 주체를 밝혀 서술하고("○○사 소개자료에 따르면 …"), 사실이나 이 과제의 계획으로 단정하지 않는다.
9. 처리를 마친 자료는 manifest에서 `processed: true`로 갱신한다.
10. 한 파일 처리에 실패하면 사유를 기록하고 다음 파일로 진행한다 (전체 중단 금지).

## 마무리

- `.llm-wiki/processing-log.md`에 append: 실행 ID, 처리 파일, 생성/갱신 문서 목록, 제안 건수, disputed 후보, 실패와 사유.
- 사람에게 요약 보고: **검토가 필요한 것**을 앞세운다 — 새 draft n건, 변경 제안 n건, 결정 후보 n건, disputed 후보 n건, 질문 n건. 각 항목에 파일 경로를 병기한다.
- 결정 후보가 있으면 후보마다 저장 명령 한 줄을 함께 준다: `llm-wiki review apply "<후보 파일명>"`. 사람에게 `40_Decisions` 파일을 직접 만들거나 복사하게 하지 않는다.
